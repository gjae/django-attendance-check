...
class DailyChecks(TimeStampedModel):
    ...
    daily = models.ForeignKey(
        DailyCalendar, on_delete=models.CASCADE, related_name="daily_user_checks"
    )
    ...
    def check_type_as_str(self):
        return (
            "ENTRADA"
            if self.checking_type == DailyChecks.CHECK_STATUS_CHOISE.entrada
            else "SALIDA"
        )
    ...
    def get_picture(self):
        if self.employee is not None:
            return (
                self.employee.picture.url
                if self.employee.picture is not None and self.employee.picture.name
                else None
            )
...
class DailyCalendarObservation(TimeStampedModel):
    ...
    support = models.FileField(
        "Soporte", upload_to="uploads/dailycalendarsupports/", null=True, default=None
    )
    check_type = models.PositiveSmallIntegerField(
        "Tipo de observación",
        choices=OBSERVATION_TYPE_CHOICES,
        default=OBSERVATION_TYPE_CHOICES.checkin_out,
    )
