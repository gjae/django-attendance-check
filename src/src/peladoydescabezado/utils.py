...
def _sheet_header(ws, turn, days, users, process):
    ...
    try:
        img = Image(
            "/app/src/static/images/branding/logo_inpromaro_lit_backup.png"
        )  # Cambia por tu archivo de imagen
        img.width = 100  # Ancho en píxeles
        img.height = 100  # Alto en píxeles
        ws.add_image(img, "A1")
    except FileNotFoundError:
        print(
            "Advertencia: No se encontró el archivo de imagen. Se insertará texto alternativo."
        )
        ws["A1"].value = "[LOGO]"
    ...
    while current_col <= end_col and part_flags:
        ...
        if current_col + block_size - 1 <= end_col:
            ...
            ws[f"{get_column_letter(current_col + block_size)}1"] = WEEKDAYS[
                current_date.weekday()
            ]
            ws[
                f"{get_column_letter(current_col + block_size)}1"
            ].alignment = center_alignment
            ws[
                f"{get_column_letter(current_col + block_size)}1"
            ].font = font_style_subheader
            ws.merge_cells(
                f"{get_column_letter(current_col + block_size)}1:{get_column_letter(current_col + block_size + 2)}2"
            )

            # sub apartado de fecha
            ws[f"{get_column_letter(current_col + block_size)}3"] = current_date.day
            ws[f"{get_column_letter(current_col + block_size + 1)}3"] = (
                current_date.month
            )
            ws[f"{get_column_letter(current_col + block_size + 2)}3"] = (
                current_date.year
            )
            ws[
                f"{get_column_letter(current_col + block_size)}3"
            ].alignment = center_alignment
            ws[
                f"{get_column_letter(current_col + block_size)}3"
            ].font = font_style_subheader
            ws[
                f"{get_column_letter(current_col + block_size + 1)}3"
            ].alignment = center_alignment
            ws[
                f"{get_column_letter(current_col + block_size + 1)}3"
            ].font = font_style_subheader
            ws[
                f"{get_column_letter(current_col + block_size + 2)}3"
            ].alignment = center_alignment
            ws[
                f"{get_column_letter(current_col + block_size + 2)}3"
            ].font = font_style_subheader
            ...
            ws.merge_cells(
                f"{get_column_letter(current_col + block_size)}3:{get_column_letter(current_col + block_size)}4"
            )
            ws.merge_cells(
                f"{get_column_letter(current_col + block_size + 1)}3:{get_column_letter(current_col + block_size + 1)}4"
            )
            ws.merge_cells(
                f"{get_column_letter(current_col + block_size + 2)}3:{get_column_letter(current_col + block_size + 2)}4"
            )
...
def _apply_font_cell_format(
    cell, font, alignment, value, width=None, letter=None, sheet=None, border=None
):
    cell.value = value
    cell.font = font
...
def _get_xlsx_report_template(
    ...
    production, users, daily_total = BasketProduction.objects.weekly_record(
        days[0], days[-1], category=category
    )
    daily_block = {}
    ...
    for i, day in enumerate(days):
        ...
        daily_block[day] = {
            "letter_start": (letter, start_letter_pick),
            "letter_end": (end_letter, start_letter_pick + 10),
        }
    ...
    if part_flags:
        for day in days:
            ...
            _apply_font_cell_format(
                ws[f"{get_column_letter(cell_name + 1)}6"],
                font_style_subheader,
                center_alignment,
                "PROCESO",
            )
            _apply_font_cell_format(
                ws[f"{get_column_letter(cell_name + 2)}6"],
                font_style_subheader,
                center_alignment,
                "TURNO",
            )
            _apply_font_cell_format(
                ws[f"{get_column_letter(cell_name + 3)}6"],
                font_style_subheader,
                center_alignment,
                "CÉDULA",
            )
            _apply_font_cell_format(
                ws[f"{get_column_letter(cell_name + 4)}6"],
                font_style_subheader,
                center_alignment,
                "FECHA",
            )
            _apply_font_cell_format(
                ws[f"{get_column_letter(cell_name + 5)}6"],
                font_style_subheader,
                center_alignment,
                "TOTAL PESADO" if category != 3 else "TOTAL CESTAS",
            )
    ...
    for i, day in enumerate(days):
        ...
        for day in days:
            ...
            for u in users:
                ...
                basket_totals += production[turn][day][str(u["identity"])][
                    "basket_count"
                ]
                ws[f"{cell}{current_totalization_row}"] = (
                    f"{production[turn][day][str(u['identity'])]['weight_sum']}"
                    if category != 3
                    else production[turn][day][str(u["identity"])]["basket_count"]
                )
                ws[f"{cell}{current_totalization_row}"].alignment = center_alignment
                ...
                total_personal[identity] = (
                    current_totalization_row,
                    total_personal[identity][1]
                    + (
                        production[turn][day][identity]["weight_sum"]
                        if category != 3
                        else production[turn][day][str(u["identity"])]["basket_count"]
                    ),
                )
            ...
            ws[f"{cell}{current_totalization_row + 1}"] = (
                weight_totals if category != 3 else basket_totals
            )
            ws[f"{cell}{current_totalization_row + 1}"].alignment = center_alignment

        ws[f"A{current_totalization_row + 1}"] = (
            "Total por día" if category != 3 else "Total por día de cestas"
        )
        ws[f"A{current_totalization_row + 1}"].font = Font(bold=True, size=14)
        ws[f"A{current_totalization_row + 1}"].alignment = Alignment(
            vertical="center", horizontal="right"
        )
        ws.merge_cells(
            f"A{current_totalization_row + 1}:C{current_totalization_row + 1}"
        )
...
def generate_rport_xlsx_simple(context):
    ...
    _apply_font_cell_format(
        ...
        font_style_subheader,
        center_alignment,
        f"CONTROL DE ENTREGA VALOR AGREGADO ({context['category_text']})",
    )
    _apply_font_cell_format(
        ws["G1"], font_style_subheader, center_alignment, f"Código: {context['code']}"
    )
    _apply_font_cell_format(
        ws["G2"],
        font_style_subheader,
        center_alignment,
        f"Fecha: {context['date'].strftime('%d/%m/%Y')}",
    )
    _apply_font_cell_format(
        ws["G3"],
        font_style_subheader,
        center_alignment,
        f"Versión: {context['version']}",
    )
    ...
    _apply_font_cell_format(
        ws["C14"], font_style_subheader, center_alignment, f"APELLIDO Y NOMBRE"
    )
    _apply_font_cell_format(
        ws["D14"],
        font_style_subheader,
        center_alignment,
        f"Contenido de cestas entregadas",
    )
    ...
    _apply_font_cell_format(
        ws[f"{content_total_col}14"],
        font_style_subheader,
        center_alignment,
        f"T. Cestas" if context["category"] == 3 else "T. Kgs",
    )
    ...
    for control in context["controls"]:
        for detail in control.control_details:
            ...
            ws[f"{col_letter}10"] = (
                f"Gramaje:  {', '.join([w.weight for w in detail.weightness.all()])}"
            )
            ws[f"{col_letter}11"] = (
                f"Kilos de camaron entero recibidos: {detail.total_weight_received}"
            )
            col_detail_start += 1
    ...
    for user in context["data"]:
        ...
        for counter in context["num_cells"]:
            ...
            col += 1
        row += 1
    ws[f"{content_total_col}{row}"] = (
        "Total Kg" if context["category"] != 3 else "Total cestas"
    )
    ws[f"{content_total_col}{row}"].alignment = Alignment(
...
def attendance_by_department_xlsx(context):
    ...
    _apply_font_cell_format(
        ...
        font_style_subheader,
        center_alignment,
        f"Departamento: {context['department'].name if context.get('department') else 'N/A'}",
    )
    ...
    o_cols = ["Cédula", "Nombre", "Apellido", "Fecha", "Descripción"]
    ...
    # body
    # body sheet 1
    cols = ["Cédula", "Nombre", "Apellido", "Cargo", "Días Marcados", "Total de horas"]
...
def attendance_by_personal_xlsx(context):
    ...
    # body
    # body sheet 1
    cols = ["Cédula", "Nombre", "Apellido", "Observaciones", "Total de horas"]
...
def get_consecutive_code(person: Person) -> str:
    ...
    max_consecutive = (
        Person.objects.aggregate(max_val=Max("consecutive"))["max_val"] or 0
    )
